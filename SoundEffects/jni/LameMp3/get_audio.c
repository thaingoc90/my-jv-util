...
#else
int lame_decode_initfile(FILE * fd, mp3data_struct * mp3data, int *enc_delay,
		int *enc_padding);
#endif
...
#endif
static FILE *open_mpeg_file(lame_t gfp, char const *inPath, int *enc_delay,
		int *enc_padding);
	 ...
	 */
	{
		...
		if (fstat(fd, &file_stat) == 0) {
			if (S_ISFIFO(file_stat.st_mode)) {
				...
				while (offset > 0) {
					size_t const bytes_to_skip = min_size_t(sizeof(buffer),
							offset);
...
RawPCMConfig global_raw_pcm = { /* in_bitwidth */16, /* in_signed   */-1, /* in_endian   */
...
int init_infile(lame_t gfp, char const *inPath) {
	...
	global_decoder.mp3input_data.stereo = 1;
	...
	setSkipStartAndEnd(gfp, enc_delay, enc_padding);
	{
		...
		if (n != MAX_U_32_NUM) {
			unsigned long const discard = global.pcm32.skip_start
					+ global.pcm32.skip_end;
 ...
 */
static int get_audio_common(lame_t gfp, int buffer[2][1152],
	...
	if (is_mpeg_file_format(global_reader.input_format)) {
		...
		if (global.snd_file) {
			...
			samples_read = read_samples_pcm(global.music_in, insamp,
...
static int unpack_read_samples(const int samples_to_read,
		const int bytes_per_sample, const int swap_order, int *sample_buffer,
		FILE * pcm_in) {
	size_t samples_read;
	int i;
...
#define GA_URS_IFLOOP( ga_urs_bps ) \
	...
	samples_read = fread(sample_buffer, bytes_per_sample, samples_to_read,
			pcm_in);
	...
	if (swap_order == 0) {
		...
		GA_URS_IFLOOP(3)
			*--op = ip[i] << (b - 24) | ip[i + 1] << (b - 16)
					| ip[i + 2] << (b - 8);
		GA_URS_IFLOOP(4)
			*--op = ip[i] << (b - 32) | ip[i + 1] << (b - 24)
...
static int read_samples_pcm(FILE * musicin, int sample_buffer[2304],
	...
	case 16:
		...
		swap_byte_order =
				(global_raw_pcm.in_endian != ByteOrderLittleEndian) ? 1 : 0;
	...
	samples_read = unpack_read_samples(samples_to_read, bytes_per_sample,
...
static int parse_wave_header(lame_global_flags * gfp, FILE * sf) {
	...
	if (is_wav) {
		...
		global.pcm_is_ieee_float =
				(format_tag == WAVE_FORMAT_IEEE_FLOAT ? 1 : 0);
...
open_mpeg_file(lame_t gfp, char const *inPath, int *enc_delay, int *enc_padding) {
	...
	if (global_reader.input_samplerate == 0) {
		(void) lame_set_in_samplerate(gfp,
				global_decoder.mp3input_data.samplerate);
	} else {
		(void) lame_set_in_samplerate(gfp, global_reader.input_samplerate);
	}
