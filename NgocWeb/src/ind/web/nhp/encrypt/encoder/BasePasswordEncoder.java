...
public abstract class BasePasswordEncoder implements IPasswordEncoder {
	...
	/**
	 * Used by sub-class to generate a merged password and salt into string. The
	 * generated string will be in the form of <code>pwdLen:password:salt</code>
	 * . The toString() method of the salt will be used to obtain the salt.
	 * 
	 * @param password
	 *            String
	 * @param salt
	...
	/**
	 * Used by sub-class to extract the password and salt from a string created
	 * using {@link #mergePasswordAndSalt(String, Object)}.
	 * 
	 * @param mergedPasswordAndSalt
	 *            String
	 * @return String[] the first element is the password and the second one is
	 *         the salt
	...
	protected String[] splitPasswordAndSalt(String mergedPasswordAndSalt) {
		...
		String password = mergedPasswordAndSalt.substring(pwdBegins + 1,
				pwdEnds);
