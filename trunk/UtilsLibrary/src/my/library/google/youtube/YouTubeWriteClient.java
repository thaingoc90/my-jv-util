...
public class YouTubeWriteClient {
	...
	/**
	 * Enum to deal with various playlist operations: VIEW = print user's
	 * playlists LIST = print contents of a playlist CREATE = create new
	 ...
	 * String's length.
	 * 
	 * @param stringToUnderline
	 *            - the string to print underlined
	 */
	 ...
	 * 
	 * @param prefix
	 *            A string to be printed before each entry.
	 * @param playlistLinkEntry
	 *            The PlaylistEntry to be printed
	 * @param showPlaylistContents
	 *            If true, show the list of videos in the playlist.
	 * @throws IOException
	 *             Error sending request or reading the feed.
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	...
	private static void printPlaylistEntry(String prefix,
		...
		System.out.printf("Description: %s\n", playlistLinkEntry.getSummary()
				.getPlainText());
		if (showPlaylistContents) {
			printPlaylist(playlistLinkEntry.getService(),
					playlistLinkEntry.getFeedUrl());
		}
	}
	...
	private static void printPlaylist(Service service, String playlistUrl)
		...
		if (playlistFeed != null) {
			for (PlaylistEntry e : playlistFeed.getEntries()) {
				System.out.println("\tPlaylist Entry: "
						+ e.getTitle().getPlainText());
				System.out.println("\tPlaylist URL: "
						+ e.getHtmlLink().getHref());
			}
		}
	}
	...
	/**
	 * Solicits the user for a video ID (hash) or tries to figure one out from
	 * the video's watch URL.
	 * 
	 * @return String containing a video ID.
	 * @throws IOException
	 *             If there are problems reading user input.
	 */
	private static String readVideoID() throws IOException {
		System.out.println("Input a valid video ID or a watch URL (default: "
				+ DEFAULT_VIDEO_ID + "):");
	 ...
	 *             If the service is unable to handle the request.
	 */
	private static void showPlaylists(YouTubeService service)
			throws IOException, ServiceException {
	 ...
	 *             If the service is unable to handle the request.
	 */
	private static void showFavorites(YouTubeService service)
			throws IOException, ServiceException {
	 ...
	 *             If the service is unable to handle the request.
	 */
	private static void showActivity(YouTubeService service)
			throws IOException, ServiceException {

		System.out
				.println("Type in a comma separated list of YouTube usernames:");
	 ...
	 * 
	 * @param service
	 *            The YouTubeService object controlling the connection to the
	 *            server.
	 * @throws IOException
	 *             Error sending request or reading the response.
	 */
	...
	private static void addComment(YouTubeService service) throws IOException {
		...
		if (videoEntry == null) {
			System.out
					.println("Sorry, the video ID you entered was not valid.\n");
			return;
		}
	...
	private static void uploadVideo(YouTubeService service) throws IOException {
		...
		System.out
				.println("What is the MIME type of this file? (ex. 'video/quicktime' for .mov)");
	 ...
	 *            An authenticated YouTubeService object.
	 * @param entry
	 *            The PlaylistLinkEntry describing the playlist to add the video
	 *            to.
	 * @throws IOException
	 ...
	 *            An authenticated YouTubeService object
	 * @param feedUrl
	 *            The url of the video feed to print.
	 * @throws IOException
	 *             Error sending request or reading the feed.
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	 ...
	 *            An authenticated YouTubeService object.
	 * @param op
	 *            The operation to perform on the playlists feed.
	 * @throws IOException
	 *             Error sending request or reading the feed.
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	...
	private static void doPlaylistFeedOperation(YouTubeService service,
		...
		if (op == PlaylistOperation.CREATE) {
			...
			newEntry.setSummary(TextConstruct.create(TextConstruct.Type.TEXT,
					description, null));
		...
		if (op == PlaylistOperation.LIST) {
			...
			if (whichVideo < 1 || whichVideo > count - 1) {
				...
				printPlaylistEntry("", playlistEntries.get(whichVideo - 1),
						true);
			}
		}
	...
	/**
	 * YouTubeWriteClient is a sample command line application that demonstrates
