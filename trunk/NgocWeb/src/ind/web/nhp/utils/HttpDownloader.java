...
public class HttpDownloader extends Downloader {
	...
	public static final int DEFAULT_NUM_OF_THREADS = 8;
			...
			if (mState == DOWNLOADING) {
				...
				if (mListDownloadThread.size() == 0) {
					if (mFileSize > MIN_DOWNLOAD_SIZE) {
						...
						int partSize = Math.round(((float) mFileSize / mNumConnections)
								/ BLOCK_SIZE)
								* BLOCK_SIZE;
						...
						HttpDownloadThread aThread = new HttpDownloadThread(1, mURL, mOutputFolder
								+ mFileName, startByte, endByte);
						mListDownloadThread.add(aThread);
						int i = 2;
						while (endByte < mFileSize) {
							startByte = endByte + 1;
							endByte += partSize;
							aThread = new HttpDownloadThread(i, mURL, mOutputFolder + mFileName,
									startByte, endByte);
							mListDownloadThread.add(aThread);
							++i;
						}
