...
int WavInFile::read(unsigned char *buffer, int maxElems) {
	...
	if (header.format.bits_per_sample != 8) {
		Log::error(
				"Error: WavInFile::read(char*, int) works only with 8bit samples.");
	}
...
int WavInFile::readRIFFBlock() {
	if (fread(&(header.riff), sizeof(WavRiff), 1, fptr) != 1)
		return -1;
...
int WavInFile::readHeaderBlock() {
	...
	if (strcmp(label, fmtStr) == 0) {
		...
		if (fread(&nLen, sizeof(int), 1, fptr) != 1)
			return -1;
		...
		// read data
		if (fread(&(header.format.fixed), nLen, 1, fptr) != 1)
			return -1;
		...
		if (fread(&(header.data.data_len), sizeof(uint), 1, fptr) != 1)
			return -1;
		...
		// read length
		if (fread(&len, sizeof(len), 1, fptr) != 1)
			return -1;
		// scan through the block
		for (i = 0; i < len; i++) {
			if (fread(&temp, 1, 1, fptr) != 1)
				return -1;
			if (feof(fptr))
...
void WavOutFile::writeHeader() {
	...
	if (res != 1) {
		Log::error("Error while writing to a wav file.");
	}
...
void WavOutFile::write(const float *buffer, int numElems) {
	...
	case 1: {
		unsigned char *temp2 = (unsigned char *) temp;
		for (int i = 0; i < numElems; i++) {
			temp2[i] = (unsigned char) saturate(buffer[i] * 128.0f + 128.0f,
					0.0f, 255.0f);
		}
		break;
	}
	...
	case 3: {
		char *temp2 = (char *) temp;
		for (int i = 0; i < numElems; i++) {
			int value = saturate(buffer[i] * 8388608.0f, -8388608.0f,
					8388607.0f);
	...
	case 4: {
		int *temp2 = (int *) temp;
		for (int i = 0; i < numElems; i++) {
			int value = saturate(buffer[i] * 2147483648.0f, -2147483648.0f,
					2147483647.0f);
